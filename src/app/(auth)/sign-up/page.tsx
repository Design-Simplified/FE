...
const LoginWithButton = ({
  ...
  handleRegister: (user: string, provider: string) => void;
}) => {
  return (
          ...
          {buttonVariant
            ...
            .map((variant: keyof typeof buttonVariantObject) => (
              <Button
                ...
                onClick={
                  buttonVariantObject[variant].text === "Sign Up with Email"
                    ? () => setClickEmail(true)
                    : () =>
                        handleRegister(changeRole ? "seller" : "buyer", variant)
                }
              >
                <div className="flex justify-center items-center w-fit flex-row gap-3">
                  <NextImage
